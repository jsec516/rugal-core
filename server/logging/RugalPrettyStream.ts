import {each, omit, isArray, isEmpty, isObject, isString} from 'lodash'
...
export class RugalPrettyStream extends Transform {
    mode: string;
    ...
    write(chunk: any, encodingOrCB?: string | Function, cb?: Function): boolean {
        ...
        if (isObject(chunk) && !(chunk instanceof Buffer)) {
     ...
     _transform(data: any, enc, cb) {
        ...
        // CASE: log objects in pretty JSON format
            ...
            try {
             ...
             each(omit(data, ['time', 'level', 'name', 'hostname', 'pid', 'v', 'msg']), (value: ILogData, key: any) => {
                // we always output errors for now
                if (isObject(value) && !isEmpty(value) && value.message && value.stack) {
                    var error = '\n';
